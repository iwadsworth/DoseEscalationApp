...
Sfun2 <- function(output1){
  ...
  if((output1[2]==1 | output1[40]==1) & output1[3]==0){ # PME failure, theta2 negative
    ...
    a <- ggplot(data.df, aes(x = doseSet, y = pj,colour="Probability of successful outcome for a given dose, p(d)")) + 
         ...
         ggtitle("Bayesian Dose response curve") + 
  ...
  b <- qplot(factor(table2[,2]), geom="bar", fill=factor(table2[,1])) + 
	     xlab("Dose (mcg/kg/min)") + ylab("Number of patients given dose") +
